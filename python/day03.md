...
- break : 종료
- continue : skip, 이후의 코드블록은 수행하지 않고, 다음 반복을 수행
- for-else
...
- return x -> None
- return 0 -> 하나를 반환, 여러 개를 원하면, 튜플을 활용(혹은 리스트를 활용)
...
- 파이썬 인터프리터에는 항상 사용할 수 있는 많은 함수와 형이 내장되어 있음
    - **map**
        ...
        - 순회 가능한 데이터 구조(iterable)의 모든 요소에 함수 적용하고, 그 결과를 map object로 반환
    ...
    - **filter**
        ...
        - iterable 데이터의 모든 요소에 함수적용, 그 결과가 True인 것들을 filter object로 반환
